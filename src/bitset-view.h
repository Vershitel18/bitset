...
public:
  ...
  bool all() const {
      ...
      if (index_begin != 0) {
        if ((*ptr_begin & (~Word{0} << index_begin)) != ~Word{0} << index_begin) { return false; }
        ++ptr_begin;
      }
      if (index_end > 0) {
        Word mask = (index_end == 64) ? ~Word{0} : (Word{1} << index_end) - 1;
        if ((*ptr_end & mask) != mask) { return false; }
        --ptr_end;
      }
      while (ptr_begin < ptr_end) {
        if (*ptr_begin != ~Word{0}) { return false; }
        ++ptr_begin;
      }
      return true;
    }
  bool any() const {
    ...
    if (index_begin != 0) {
      ...
      if ((*ptr_begin & mask) != 0) { return true; }
      ++ptr_begin;
    }
    if (index_end > 0) {
      Word mask = (index_end == 64) ? ~Word{0} : (Word{1} << index_end) - 1;
      if ((*ptr_end & mask) != 0) { return true; }
      --ptr_end;
    }
    while (ptr_begin <= ptr_end) {
      if (*ptr_begin != 0) { return true; }
      ++ptr_begin;
    }
    return false;
  }
  ...
  BitSetView subview(std::size_t offset = 0, std::size_t count = NPOS) const {
    ...
    else if (count == NPOS || offset + count > size()) { count = size() - offset; }
  ...
  const View& apply(Op op) const {
    ...
    if (ptr_begin == ptr_end) {
      Word mask =
          (~Word{0} << idx_begin) &
          ((idx_end == 64) ? ~Word{0} : ((Word{1} << idx_end) - 1));
    ...
    const View& binary_operation(const const_view& other, Operation op) const {
      ...
      if (ptr1 == end_ptr1) {
        Word mask = (~Word{0} << idx1) &
                    ((end_idx1 == 64) ? ~Word{0} : (Word{1} << end_idx1) - 1);
    ...
    const View& operator&=(const const_view& other) const {
      return binary_operation(other, [](Word a, Word b) { return a & b; });
    }
    const View& operator|=(const const_view& other) const {
      return binary_operation(other, [](Word a, Word b) { return a | b; });
    }
